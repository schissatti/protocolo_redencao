...
// LocalStorage Keys
const STORAGE_COMPLETED_KEY = 'truque_mulher_completed_lessons_es';
const STORAGE_UNLOCKED_KEY = 'truque_mulher_unlocked_modules_es';
...
function updateGamificationUI() {
  ...
  let currentLevel = 'Novato en la Cama';
  let nextLevel = 'Seductor Nato';
  let nextTarget = 20;

  if (percent >= 100) {
    currentLevel = 'Maestro de la Seducción';
    nextLevel = 'Nivel Máximo Alcanzado';
    nextTarget = 100;
  } else if (percent >= 80) {
    ...
    currentLevel = 'Seductor';
    nextLevel = 'Conquistador';
    nextTarget = 40;
  } else {
    currentLevel = 'Novato en la Cama';
    nextLevel = 'Seductor Nato';
    nextTarget = 20;
  }
  ...
  if (percent >= 100) {
    badgeEl.textContent = 'Mapeo 100% Completo!';
  } else {
    badgeEl.textContent = `Faltan ${needed}% para desbloquear`;
  }
...
function renderModulesGrid() {
  ...
  if (filtered.length === 0) {
    grid.innerHTML = `<div style="grid-column: 1 / -1; text-align: center; padding: 40px; color: var(--text-muted);">
      Ningún módulo o contenido encontrado para "${currentSearchQuery}".
    </div>`;
    return;
  }
      ...
      <div class="card-body">
        ...
        <div class="card-footer-stats">
          <div class="card-progress-row">
            <span class="card-progress-label">Progreso del Módulo</span>
            <span class="card-progress-val">${modProgress}%</span>
          </div>
          <div class="card-action-btn">
            ${isUnlocked ? '<span>Acceder al Módulo</span> <span>→</span>' : '<span>🔒 Desbloquear Contenido VIP</span> <span>→</span>'}
          </div>
        </div>
      </div>
    `;
...
function selectLesson(lesson) {
  ...
  if (lesson.type === 'video') {
    ...
    newVideo.onerror = () => {
      wrapper.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; height:100%; padding:20px; text-align:center; color:#fff; gap:12px;">
          ...
          <h4 style="font-size:16px; font-weight:700;">Vídeo de la Lección</h4>
          <p style="font-size:12px; color:var(--text-muted); max-width:400px;">
            Haga clic abajo para ver la videoclase directamente en alta definición.
          </p>
          <a href="${encodedPath}" target="_blank" class="pdf-action-link primary" style="padding:10px 20px; font-size:13px;">
            <span>▶</span> Abrir Videoclase en Nueva Pestaña
          </a>
        </div>
      `;
    };
    ...
    wrapper.innerHTML = `
      ...
      <h3 class="spotify-heading">Playlist Oficial de Música Hot (Spotify)</h3>
      <p class="spotify-subheading">
        Acceda a la selección musical exclusiva desarrollada para crear el clima y la atmósfera perfecta durante el Protocolo del Nexo.
      </p>
      <a href="${spotifyUrl}" target="_blank" class="spotify-btn">
        <span>🟢</span> ACCEDER A PLAYLIST EN SPOTIFY
      </a>
    ...
    wrapper.innerHTML = `
      <div class="pdf-toolbar">
        <div class="pdf-toolbar-title">
          <span>📖 Lector de E-book VIP</span>
        </div>
        <div class="pdf-toolbar-actions">
          <a href="${encodedPath}" target="_blank" class="pdf-action-link" title="Abrir en Nueva Pestaña">
            <span>🔗</span> Nueva Pestaña
          </a>
          <a href="${encodedPath}" download class="pdf-action-link primary" title="Descargar PDF">
